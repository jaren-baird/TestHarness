...
public class HarnessAutoBackup 
{
	...
	public static void main(String[] args) 
    {
		...
		try 
		{
			...
			}, new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse("2016-2-25 20:00:00"), 3*60*60*1000); //Repeat time in milliseconds (8*60*60*1000) 8 hours
		} 
			...
			if(result == -999 || result == -998)
			{
				...
				try
				{
					...
					else if(result == -998)
					{
						...
						connection = keyboard.nextInt(); //connection = 1;
						//Thread.sleep(120000);
						if(connection == 0)
	...
	public static int RunJar(String FilePath)
	{
		...
		try
		{
			if(debug != 1)
			{
				...
				}while(result == 100 && retry < 8 || result == 1 && retry < 20);
			}
		}
		catch (Exception ex)
		{
	...
	{
		Process process = null;
		String path = TEST_DIRNAME+"TCResult\\TCResults.exe";
		String resultString = "";
